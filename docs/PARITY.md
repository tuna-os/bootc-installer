...
| Recovery key page after TPM enrolment | yes | log only | yes | log only | yes |
...
3. **Recovery key page** (KDE, Niri): fisherman prints the key after TPM
   enrolment. GNOME shows a page. XFCE and COSMIC show a panel on the done
   screen. Each holds the restart button until the user acknowledges the
   key. The other two write it to the install log only (#129).
